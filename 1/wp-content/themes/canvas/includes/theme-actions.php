...
function woo_custom_styling() {
	...
	if ( isset( $woo_options['woo_style_disable'] ) && $woo_options['woo_style_disable'] != 'true' ) {} else {
		...
		if ( $nav_font )
			$output .= 'ul.nav li a, #navigation ul.rss a, #navigation .cart-contents, #navigation ul.rss, #navigation ul.cart .cart_list a{ ' . woo_generate_font_css( $nav_font, 1.2 ) . ' } #navigation ul.rss li a:before { color:' . $nav_font['color'] . ';}' . "\n";
		if ( $nav_hover )
		...
		// Slider overlay
		if ( $slider_biz_overlay == 'center' || $slider_biz_overlay == 'full' || $slider_biz_overlay == 'none' ) {
			$output .= '@media only screen and (min-width:768px) {' . "\n";
		...
		<ul class="slides"<?php if ( '' != $args['height'] ): ?> style="height: <?php echo $args['height']; ?>px;"<?php endif; ?>>
			...
			<li id="slide-<?php echo esc_attr( $post->ID ); ?>" class="slide slide-number-<?php echo esc_attr( $count ); ?>" <?php if ( '' != $args['slide_styles'] ): ?>style="<?php echo esc_attr( $args['slide_styles'] ); ?>"<?php endif; ?>>
				...
				?>
					...
					<?php woo_image( 'width=' . $args['width'] . '&height=' . $args['height'] . '&link=img&noheight=true' ); ?>
