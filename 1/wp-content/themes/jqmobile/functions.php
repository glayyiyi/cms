...
function jquerymobile_enquee_script() {
	...
	wp_enqueue_script('jquerymobile', get_stylesheet_directory_uri().'/jquerymobile/jquery.mobile-1.3.2.min.js', array('jquery'), '1.0');
}
add_action('wp_enqueue_scripts', 'jquerymobile_enquee_script');

function jquerymobile_enquee_style() {
    wp_enqueue_style('jquerymobile', get_stylesheet_directory_uri().'/jquerymobile/jquery.mobile.structure-1.3.2.min.css', false, '1.0');
		...
		<div id="comment-<?php comment_ID(); ?>" class="comment vcard ui-link-inherit">
				...
				<div class="ui-li-heading">
					<?php
						...
						printf( __( '%1$s 在 %2$s <span class="says">回复:</span>', 'jquerymobile' ),
							sprintf( '<span class="fn">%s</span>', get_comment_author_link() ),
							sprintf( '<a href="%1$s"><span title="%2$s">%3$s</span></a>',
								esc_url( get_comment_link( $comment->comment_ID ) ),
								get_comment_time( 'c' ),
								/* translators: 1: date, 2: time */
								sprintf( __( '%1$s 于 %2$s', 'jquerymobile' ), get_comment_date( 'Y/m/d' ), get_comment_time() )
							)
						);
					?>

					<?php edit_comment_link( __( '编辑', 'jquerymobile' ), '<span class="edit-link">', '</span>' ); ?>
