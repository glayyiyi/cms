            ...
            function wp_version_notice(){
                ...
                if ( version_compare($wp_version, '3.5', '<') ) {
                    echo '<div class="updated">
					   <p>Page Builder requires WP version 3.5+. <strong>Please, Update Wordpress.</strong></p>
					</div>';
                }
            }
            add_action('admin_notices', 'wp_version_notice');

            $this->page = add_menu_page($this->args['page_title'], $this->args['menu_title'],  'manage_options', $this->args['page_slug'], array(&$this, 'builder_settings_show'), $this->url.'/home_page_builder_ico.png', 4);

            //enqueueu styles/scripts on the builder page
            add_action('admin_print_styles-'.$this->page, array(&$this, 'admin_enqueue'));

            //add_menu_page($this->args['page_title'], $this->args['menu_title'],  'manage_options', $this->args['page_slug'], array(&$this, 'builder_settings_show'), plugins_url('myplugin/images/icon.png'), 26);
        ...
        function display_template($template_id) {
            ...
            if(empty($blocks)) {
                ...
                foreach($blocks as $key => $instance) {
                    ...
                    if (isset($instance['shortcode'])) $instance['shortcode'] = stripslashes($instance['shortcode']);
...
//notice function if version is lower than 3.5
